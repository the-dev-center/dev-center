...
DevCentr may offer an **SSH Manager** that acts as a master coordinator: it lists keys from your configured agents (e.g. Bitwarden, OpenSSH), launches their login/unlock GUIs when needed, and forwards sign requests to them so Git and SSH work from one place. See xref:specifications:components/ssh-manager.adoc[SSH Manager] in the specifications.
