...
3. Install **Git** using `winget install Git.Git` (see xref:reference/git-installation.adoc[Git Installation] for package advice).
4. Enable **sudo for Windows** in your system settings.
...
+
include::partial$git-init-defaultbranch.adoc[]
...
Your system uses Corepack to manage package managers. Corepack acts as a wrapper that ensures the version of pnpm used matches what the project expects.
