...
So we plug in by documenting or shipping a **WSL bridge** so that `SSH_AUTH_SOCK` in WSL points at a socket that eventually reaches our Windows agent (or the user's chosen Windows agent). The existing xref:knowledge-base:explanation/infrastructure/ssh-keys.adoc[SSH Keys] doc already notes that WSL users need a bridge to use the Windows ssh-agent; the same idea applies when our SSH Manager is the coordinator.
